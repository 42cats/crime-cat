...
const { editPasswordContent } = require('../../Commands/api/passwordNote/passwordNote');

module.exports = {
	name: "passwordNoteEdit", // 반드시 encodeToString()에서 사용된 command와 같아야 함
	...
	execute: async (client, interaction) => {
		...
		try {
			...
			let ret = null;
			try {
				ret = await editPasswordContent(guildId, channelId, otherOption ,passwordKey, content);
			}
			catch (e) {
				console.log(e);
			...
			if (ret){
				const channel = interaction.guild.channels.cache.get(ret.channelSnowflake);
