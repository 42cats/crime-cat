...
class MusicActionExecutor extends BaseActionExecutor {
    ...
    async performAction(action, context) {
        ...
        try {
            ...
            switch (type) {
                case 'play_music':
                    logger.debug('playMusic 호출 options:', {
                        source: action.parameters.source,
                        trackId,
                        trackTitle,
                        playMode
    ...
    async playMusic(searchQuery, voiceChannel, textChannel, options = {}, context = null) {
        logger.debug('playMusic 메서드 시작 - options:', {
        ...
        try {
            ...
            if (options.trackId && options.trackTitle) {
                ...
                logger.info(`사전 선택된 트랙 사용: "${track.title}" (${track.id})`);
            } else {
                // searchQuery로 검색
                ...
                logger.info(`검색으로 트랙 발견: "${track.title}"`);
            }
            ...
            logger.debug('musicService.playMusic 호출 전 - 최종 options:', {
        ...
        return {
            ...
            playMusic: async (voiceChannel, textChannel, track, options) => {
                try {
                    ...
                    if (!musicData) {
                        logger.info(`새 음악 플레이어 생성: ${guild.id}`);
                    ...
                    if (musicData.queue.source !== sourceToLoad) {
                        logger.info(`소스 전환: ${musicData.queue.source || 'none'} -> ${sourceToLoad}`);
                    } else {
                        logger.debug(`소스 재확인: ${sourceToLoad}`);
                    }
                    
                    // 항상 지정된 소스로 로드 (캐시된 데이터가 있어도 새로 로드)
                    logger.info(`${sourceToLoad} 소스 강제 로드 시작`);
                    const loaded = await musicData.queue.loadFromSource(sourceToLoad, member.user.id);
                    logger.debug('소스 로드 결과:', {
                        성공: loaded,
                    ...
                    logger.info(`트랙 찾음: 인덱스 ${trackIndex}, 제목: "${musicData.queue.tracks[trackIndex].title}"`);
                    ...
                    logger.info(`음악 재생 시작: ${currentTrack.title}`);
                    
                    // 재생 모드 설정 (파라미터로 받은 값 또는 기본값 single-track)
                    await musicData.setMode(selectedPlayMode);
                    logger.info(`재생 모드를 ${selectedPlayMode}로 설정`);
                    ...
                    if (options.duration && options.duration > 0) {
                        setTimeout(() => {
                            if (musicData.state.isPlaying) {
                                logger.info(`재생 시간 만료로 음악 정지: ${options.duration}초`);
