...
const { USER_PERMISSION, hasPermission } = require('./UserGrade');
...
class AudioPlayerManager {
    ...
    async join(operater) {
        ...
        if (!this.connection) {
            ...
            if (!voiceChannel)
                throw new Error("음성채널에 접속해 있지 않음!");
    ...
    async play(track, onStateChange) {
        ...
        if (fs.existsSync(track.url)) {
            ...
            const beatRate = hasPermission(this.parent.operater, USER_PERMISSION.AUDIO_BIT_RATE) ? 'bestaudio' : 'bestaudio[abr<=128]';
    ...
    destroy() {
        try {
            ...
            throw new Error('리소스 정리 중 오류가 발생했습니다.');
        }
    }
}
