...
module.exports = {
    ...
    prefixCommand: {
        ...
        async execute(message, args) {
            if (args.length === 0) {
                const msg = await message.channel.send("입력 예)\`\`\`!계산 14+(21*34) -1 /2 **2\`\`\`");
...
function calculateExpression(expression) {
    ...
    const operators = [
        ...
        { regex: /[*\/]/g, precedence: 2, operation: (a, b, op) => (op === '*' ? a * b : a / b) },
        { regex: /[+\-]/g, precedence: 1, operation: (a, b, op) => (op === '+' ? a + b : a - b) },
    ];
    ...
    const processTokens = (tokens) => {
        ...
        for (const { regex, operation } of operators) {
            ...
            while (i < tokens.length) {
                if (typeof tokens[i] === 'string' && regex.test(tokens[i])) {
                    ...
                    if (isNaN(left) || isNaN(right)) throw new Error('인식 불가능한 표현식이 있습니다');
        ...
        if (tokens.length !== 1) throw new Error('인식 불가능한 표현식이 있습니다.');
        return tokens[0];
    };
