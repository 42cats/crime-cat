...
prefixCommand: {
	...
	async execute(message, args) {
		...
		try {
			const response = await editPermisson(permissionName, price, duration);
			await message.reply(
				`✔ 권한 수정 완료\n권한 이름: ${permissionName}\n가격: ${price}\n기간: ${duration}일 ${response.data.message}`
			);
		} catch (error) {
			console.error('권한 수정 실패:', error);
			await message.reply('❌ 권한 수정 중 오류가 발생했습니다.');
		}
	}
	},
	upload: false,
