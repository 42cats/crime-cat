...
const SNSExplorePage: React.FC = () => {
  ...
  const loadPosts = useCallback(async (resetPage = false) => {
    ...
    try {
      ...
      if (isSearching && searchQuery.startsWith('#')) {
        ...
        // 인기 게시물
        postsData = await exploreService.getPopularPosts(currentPage);
      } else {
        // 무작위 게시물
        postsData = await exploreService.getRandomPosts(currentPage);
      }
  ...
  const loadPopularHashtags = useCallback(async () => {
    try {
      ...
      setPopularHashtags(hashtags.slice(0, 10)); // 상위 10개만 표시
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      {!isSearching && popularHashtags.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-medium text-muted-foreground mb-2">인기 해시태그</h2>
      ...
      {/* 탭 (검색 중이 아닐 때만 표시) */}
      {!isSearching && (
        <Tabs defaultValue="popular" value={activeTab} onValueChange={handleTabChange} className="mb-6">
