...
const GameComparisonPage: React.FC = () => {
  ...
  const [selectedUsers, setSelectedUsers] = useState<Array<{ id: string; nickname: string; avatar?: string }>>([]);
  const [isSearchModalOpen, setIsSearchModalOpen] = useState(false);
  const [searchKeyword, setSearchKeyword] = useState('');
  const [activeTab, setActiveTab] = useState<'crimescene' | 'escaperoom'>('crimescene');
  const [comparisonFilter, setComparisonFilter] = useState<Partial<GameComparisonRequest>>({
  ...
  // 게임 비교 조회
  const { data: comparisonData, isLoading: isComparing, refetch } = useQuery<GameComparisonResponse>({
    queryKey: ['game-comparison', selectedUsers.map(u => u.id), activeTab, comparisonFilter],
    queryFn: () => gameComparisonService.compareGameHistories({
      userIds: [user!.id, ...selectedUsers.map(u => u.id)],
      gameType: activeTab === 'crimescene' ? GameType.CRIMESCENE : GameType.ESCAPE_ROOM,
      ...comparisonFilter,
    } as GameComparisonRequest),
    enabled: selectedUsers.length > 0 && !!user,
  });

  const handleAddUser = (userToAdd: { id: string; nickname: string; avatar?: string }) => {
  ...
  return (
    <div className="container mx-auto py-6 px-4 max-w-7xl">
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="text-lg">비교할 사용자 선택</CardTitle>
          <CardDescription>
            최대 10명까지 선택할 수 있습니다
          </CardDescription>
        </CardHeader>
      ...
      {selectedUsers.length === 0 ? (
        ...
        <Card>
          <CardContent className="py-20">
            <div className="flex flex-col items-center justify-center">
              ...
              <p className="text-muted-foreground">게임 기록을 비교하는 중...</p>
            </div>
          </CardContent>
        </Card>
      ) : comparisonData ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {Object.entries(comparisonData.userStatistics).map(([userId, stats]) => (
              <Card key={userId}>
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">{stats.nickname}</p>
                      {userId === user?.id && (
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">플레이</span>
                      <span className="font-medium">{stats.totalPlayCount}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">고유 테마</span>
                      <span className="font-medium">{stats.uniqueThemeCount}</span>
                    </div>
                    <div>
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange}>
            ...
            <TabsContent value="crimescene" className="space-y-4 mt-4">
              <ComparisonResults
                themes={comparisonData.unplayedThemes.filter(t => t.gameType === GameType.CRIMESCENE)}
                totalThemeCount={comparisonData.totalThemeCount}
                commonUnplayedCount={comparisonData.commonUnplayedCount}
            ...
            <TabsContent value="escaperoom" className="space-y-4 mt-4">
              <ComparisonResults
                themes={comparisonData.unplayedThemes.filter(t => t.gameType === GameType.ESCAPE_ROOM)}
                totalThemeCount={comparisonData.totalThemeCount}
                commonUnplayedCount={comparisonData.commonUnplayedCount}
                onThemeClick={handleThemeClick}
                pageInfo={comparisonData.pageInfo}
...
}> = ({ themes, totalThemeCount, commonUnplayedCount, onThemeClick, pageInfo, onPageChange }) => {
  return (
    <>
      {/* 요약 통계 */}
      <div className="grid grid-cols-3 gap-4 mb-4">
        ...
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">공통 미플레이</p>
            <p className="text-2xl font-bold text-orange-600">{themes.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">추천 테마</p>
            <p className="text-2xl font-bold text-green-600">
              {themes.filter(t => t.recommendations > 50).length}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      {themes.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-lg font-medium mb-2">모든 테마를 플레이했습니다!</p>
            <p className="text-muted-foreground">
              선택한 사용자들이 공통으로 플레이하지 않은 테마가 없습니다.
            </p>
          </CardContent>
        </Card>
      ) : (
