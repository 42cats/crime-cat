...
const CommandDetail: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['command', id],
    queryFn: () => id ? commandsService.getCommandById(id) : Promise.reject('No ID'),
    enabled: !!id,
  });
  ...
  const handleEdit = () => navigate(`/commands/edit/${id}`, { state: { command } });
    ...
    return (
      <PageTransition>
        <div className="container mx-auto px-6 py-20 text-center">
          <h1 className="text-2xl font-bold mb-4">명령어를 찾을 수 없습니다</h1>
          <Button onClick={() => navigate('/commands')}>목록으로 돌아가기</Button>
        </div>
      </PageTransition>
    );
  }
    ...
    <PageTransition>
      <div className="container mx-auto px-6 py-20">
        <div className="max-w-3xl mx-auto space-y-8">
          {/* Back */}
          <Link to="/commands" className="inline-flex items-center text-muted-foreground hover:text-primary">
          ...
          {command.requiredPermissions?.length > 0 && (
            <section className="mt-6">
              <h2 className="text-sm font-semibold text-muted-foreground mb-1">필수 권한</h2>
          ...
          {/* 본문 */}
          <section className="mt-6">
            <h2 className="text-sm font-semibold text-muted-foreground mb-2">내용</h2>
