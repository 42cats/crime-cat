...
const ThemeAdvertisements: React.FC = () => {
    ...
    const getThemeTypeBadge = (type: ThemeAdvertisementRequest["themeType"]) => {
        const typeConfig = {
    ...
    const handleAdvertisementRequest = async (data: CreateAdvertisementRequest) => {
        try {
    ...
    const handleCancelAdvertisement = async (request: ThemeAdvertisementRequest) => {
        try {
            const response = request.status === "ACTIVE" 
                ? await cancelActiveAdvertisement(request.id)
                : await cancelQueuedAdvertisement(request.id);
    ...
    const completedAds = requests.filter(req => 
        req.status === "EXPIRED" || req.status === "CANCELLED" || req.status === "REFUNDED"
    );
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="space-y-2">
                                <div className="flex items-center justify-between text-sm">
                                    <span>활성 슬롯</span>
                                    <span>{queueStatus.activeCount}/{queueStatus.maxActiveSlots}</span>
                                </div>
                                <Progress 
                                    value={(queueStatus.activeCount / queueStatus.maxActiveSlots) * 100} 
                                    className="h-2"
                                />
                            </div>
                            <div className="text-center">
                                ...
                                <div className="text-sm text-muted-foreground">대기 중인 광고</div>
                            </div>
                            <div className="text-center">
                                <div className="text-lg font-semibold">
                                    {queueStatus.estimatedWaitTime}
                                </div>
                                <div className="text-sm text-muted-foreground">예상 대기시간</div>
                            </div>
                        </div>
                        
                        {queueStatus.activeCount >= queueStatus.maxActiveSlots && (
                            <Alert className="mt-4">
                                <AlertCircle className="h-4 w-4" />
                                <AlertTitle>광고 슬롯 만료</AlertTitle>
                                <AlertDescription>
                                    현재 모든 광고 슬롯이 사용 중입니다. 새로운 광고는 대기열에 추가됩니다.
                                </AlertDescription>
                            </Alert>
                        )}
                    </CardContent>
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-3">
                                                <h3 className="text-lg font-semibold">{ad.themeName}</h3>
                                                {getThemeTypeBadge(ad.themeType)}
                                        ...
                                        <Button 
                                            variant="outline" 
                                            size="sm"
                                            onClick={() => handleCancelAdvertisement(ad)}
                                        >
                                ...
                                <CardContent>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div className="space-y-2">
                                            <div className="flex items-center justify-between text-sm">
                                                <span>진행 상황</span>
                                                <span>{ad.remainingDays}일 남음</span>
                                            </div>
                                            <Progress 
                                                value={calculateProgress(ad.remainingDays!, ad.requestedDays)}
                                                className="h-2"
                                            />
                                            <div className="text-xs text-muted-foreground">
                                                {ad.requestedDays - ad.remainingDays!}일 / {ad.requestedDays}일 진행
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-semibold">{ad.exposureCount.toLocaleString()}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-semibold">{ad.totalCost.toLocaleString()}P</span>
                                            </div>
                                            <div className="text-xs text-muted-foreground">
                                                만료일: {new Date(ad.expiresAt!).toLocaleDateString()}
                                            </div>
                                        </div>
                                    </div>
                                </CardContent>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div className="space-y-2">
                                            <div className="text-center">
                                                ...
                                                <div className="text-sm text-muted-foreground">대기 순번</div>
                                            </div>
                                        </div>
                                        
                                        <div className="space-y-2">
                                            <div className="text-sm">신청 기간: {ad.requestedDays}일</div>
                                            <div className="text-sm">
                                                신청일: {new Date(ad.requestedAt).toLocaleDateString()}
                                            </div>
                                        </div>
                        ...
                        completedAds.map(ad => (
                            <Card key={ad.id}>
                                ...
                                <CardContent>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div className="space-y-2">
                                            <div className="text-sm">기간: {ad.requestedDays}일</div>
                                            <div className="text-sm">
                                                완료일: {ad.expiresAt ? new Date(ad.expiresAt).toLocaleDateString() : '-'}
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2">
                                                <Eye className="w-4 h-4 text-blue-500" />
                                                <span className="text-sm">총 노출수</span>
                                                <span className="font-semibold">{ad.exposureCount.toLocaleString()}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <MousePointer className="w-4 h-4 text-green-500" />
                                                <span className="text-sm">총 클릭수</span>
                                                <span className="font-semibold">{ad.clickCount.toLocaleString()}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {ad.refundAmount && (
                                                <div className="text-xs text-green-600">
                                                    환불: {ad.refundAmount.toLocaleString()}P
                                                </div>
                                            )}
                                        </div>
                                    </div>
