...
// 유효성 검사 유틸리티
import { validateInstagramUrl, validateTwitterUrl, validateDiscordUrl } from "@/utils/validators";
...
const Profile: React.FC = () => {
  ...
  useEffect(() => {
    const loadProfileData = async () => {
      ...
      if (profileData) {
        ...
        setCroppedImageUrl(profileData.avatar || "/default-profile.jpg");
        
        // 소셜 링크 설정
        if (profileData.social_links) {
          setInstagram(profileData.social_links.instagram || "");
          setTwitter(profileData.social_links.x || "");
          setDiscord(profileData.social_links.openkakao || "");
          
          // 초기 URL 유효성 검사
          validateInstagramUrlWrapper(profileData.social_links.instagram || "");
          validateTwitterUrlWrapper(profileData.social_links.x || "");
          validateDiscordUrlWrapper(profileData.social_links.openkakao || "");
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 닉네임 유효성 확인
    if (!isNicknameValid || (!nicknameChecked && nickname !== originalData?.nickname)) {
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        >
          ...
          <p className="text-muted-foreground">
            계정 설정을 통해 프로필을 업데이트하고 기본 설정을 변경하세요.
          </p>
        </motion.div>
          ...
          <form onSubmit={handleSubmit}>
                ...
                <CardContent className="pt-6 space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <SocialLinks 
                      ...
                      validateInstagramUrl={validateInstagramUrlWrapper}
                      validateTwitterUrl={validateTwitterUrlWrapper}
                      validateDiscordUrl={validateDiscordUrlWrapper}
                      isDark={isDark}
                    />
                  </div>
                </CardContent>
            ...
            <TabsContent value="notifications" className="space-y-6">
              <NotificationSettings 
                ...
                setNotifyByDiscord={handleDiscordNotificationChange}
                isDark={isDark}
              />
            </TabsContent>
          </form>
        ...
        <Dialog open={showDeleteModal} onOpenChange={setShowDeleteModal}>
          <DialogContent>
            ...
            <div className="py-4 space-y-2">
              ...
              <p className="text-sm text-muted-foreground">계속 진행하시겠습니까?</p>
