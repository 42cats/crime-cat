...
import { 
  ArrowLeft, Heart, Edit, Trash2, Image, Share2, 
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: result ? '포스트를 좋아합니다.' : '좋아요를 취소했습니다.',
      });
    } catch (error) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-96">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-6">{error || '요청한 포스트가 존재하지 않거나 접근할 수 없습니다.'}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold tracking-tight">포스트 상세</h1>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button 
            variant="secondary" 
            size="sm" 
            onClick={() => navigate(`/dashboard/posts/edit/${post.postId}`)}
          >
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              ...
              <div>
                <div className="font-medium">{post.authorNickname}</div>
              </div>
            </div>
