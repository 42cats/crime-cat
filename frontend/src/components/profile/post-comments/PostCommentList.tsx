...
const PostCommentList: React.FC<PostCommentListProps> = ({
  ...
  const fetchComments = async (page = 0, sort: CommentSortType = "LATEST", size = pageSize) => {
    ...
    setIsLoading(true);
    try {
      const result = await userPostCommentService.getComments(postId, page, size, sort);
  ...
  const fetchMoreComments = useCallback(async () => {
    ...
    try {
      ...
      const result = await userPostCommentService.getComments(postId, nextPage, pageSize, sortType);
  ...
  // 답글 생성 핸들러
  const handleReplyComment = async (parentId: string, data: UserPostCommentRequest) => {
    if (!currentUserId) {
      onLoginRequired();
      return;
    }
  ...
  // 댓글 수정 핸들러
  const handleUpdateComment = async (commentId: string, data: UserPostCommentRequest) => {
    if (!currentUserId) {
      onLoginRequired();
      return;
    }
