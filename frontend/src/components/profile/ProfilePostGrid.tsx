...
import { userPostService, UserPostGalleryDto, UserPostDto } from '@/api/userPost/userPostService';
  ...
  useEffect(() => {
    if (selectedPostId) {
      const fetchPostDetail = async () => {
        try {
          const data = await userPostService.getUserPostDetail(selectedPostId);
  ...
  if (loadError) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-gray-400 bg-gray-50 rounded-md min-h-[150px] md:min-h-[200px]">
        ...
        <p className="text-sm font-medium mb-2">{loadError.type === 'not_found' ? '포스트를 찾을 수 없습니다' : '오류가 발생했습니다'}</p>
            ...
            {post.thumbnailUrl ? (
                ...
                <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/70 to-transparent text-white">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <Heart size={16} className={post.liked ? 'text-red-500' : 'text-gray-300'} />
              ...
              <div className="aspect-square p-3 flex flex-col justify-between">
                <div className="line-clamp-6 text-sm font-medium">
                  ...
                  <span className="text-gray-500">포스트를 확인하려면 클릭하세요</span>
                </div>
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center space-x-2">
                    <Heart size={16} className={post.liked ? 'text-red-500' : 'text-gray-500'} />
                    <span className="text-xs">{post.likeCount}</span>
                  </div>
                  <button
                    onClick={(e) => handleShare(post.postId, e)}
