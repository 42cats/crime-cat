    ...
    useEffect(() => {
        const fetchThemeDetail = async () => {
            if (isOpen && theme.themeId) {
                ...
                try {
                    const data = await themesService.getThemeById(
                        theme.themeId
                    );
                    setThemeDetail(data);
                    // 메타데이터 업데이트
    ...
    useEffect(() => {
        const checkLikeStatus = async () => {
            if (isOpen && theme.themeId && isAuthenticated) {
                try {
                    const likeStatus = await themesService.getLikeStatus(
                        theme.themeId
                    );
                    setLiked(likeStatus);
                } catch (error) {
    ...
    return (
        <>
            <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
                <DialogContent className="max-w-5xl w-[95%] md:w-full bg-white rounded-lg p-0 overflow-hidden">
                    <DialogTitle className="sr-only">
                        테마 상세 정보
                    </DialogTitle>

                    {/* 모바일/데스크탑 레이아웃 컨테이너 */}
                    <div className="h-[85vh] md:h-[80vh] overflow-y-auto md:overflow-hidden md:flex md:flex-col">
                        {/* 반응형 레이아웃: 모바일과 데스크탑 */}
