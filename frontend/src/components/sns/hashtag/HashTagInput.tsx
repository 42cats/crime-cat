...
const HashTagInput: React.FC<HashTagInputProps> = ({ value, onChange }) => {
    ...
    useEffect(() => {
        const searchHashTags = async () => {
            ...
            if (match) {
                ...
                if (query.length > 0) {
                    try {
                        const hashTags = await hashtagService.searchHashTags(
                            query
                        );
    ...
    const handleSelectHashTag = (tagName: string) => {
        ...
        if (match) {
            const newValue =
                beforeCursor.replace(hashTagRegex, `#${tagName}`) + afterCursor;
            onChange(newValue);
            setShowSuggestions(false);
    ...
    return (
        <div className="relative">
            ...
            <div className="hidden">{renderHighlightedContent()}</div>
        </div>
    );
};
