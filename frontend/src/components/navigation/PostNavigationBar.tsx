    ...
    <div className="w-full">
      ...
      <div className="hidden md:flex lg:hidden items-center justify-center gap-3">
        <button
          ...
          aria-label={navigation?.previousPost ? `이전글: ${navigation.previousPost.subject}` : '이전글 없음'}
        >
          <ChevronLeft className="w-4 h-4 flex-shrink-0" />
          <span className="text-sm truncate">
            {navigation?.previousPost?.subject || '이전글'}
          </span>
        </button>
        ...
        <button
          ...
          aria-label={navigation?.nextPost ? `다음글: ${navigation.nextPost.subject}` : '다음글 없음'}
        >
          <span className="text-sm truncate">
            {navigation?.nextPost?.subject || '다음글'}
          </span>
      ...
      <div className="flex md:hidden flex-col gap-3">
        ...
        <div className="flex items-center justify-center gap-2">
          <button
            ...
            title={navigation?.previousPost ? `이전글: ${navigation.previousPost.subject}` : '이전글 없음'}
            aria-label={navigation?.previousPost ? `이전글: ${navigation.previousPost.subject}` : '이전글 없음'}
          >
          ...
          <button
            ...
            disabled={!navigation?.nextPost || isLoading}
            className="flex items-center justify-center w-12 h-12 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-offset-2 active:scale-95"
            title={navigation?.nextPost ? `다음글: ${navigation.nextPost.subject}` : '다음글 없음'}
            aria-label={navigation?.nextPost ? `다음글: ${navigation.nextPost.subject}` : '다음글 없음'}
          >
        ...
        <div className="space-y-2">
          {navigation?.previousPost && (
            <div 
              className="text-xs text-gray-600 dark:text-gray-400 p-2 bg-gray-50 dark:bg-gray-800 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
              onClick={onPreviousPost}
            >
              <div className="flex items-center gap-1">
                <ChevronLeft className="w-3 h-3" />
                <span className="font-medium">이전:</span>
                <span className="truncate">{navigation.previousPost.subject}</span>
              </div>
            </div>
          )}
          ...
          {navigation?.nextPost && (
            ...
            >
              <div className="flex items-center gap-1">
                ...
                <span className="truncate">{navigation.nextPost.subject}</span>
              </div>
            </div>
          )}
        </div>
