...
import {
    escapeRoomCommentService,
    CommentResponse,
} from "@/api/comment/escapeRoomCommentService";
...
const GameHistorySection: React.FC<GameHistorySectionProps> = ({
    ...
    const { data: historiesData, isLoading } = useQuery({
        ...
        queryFn: () =>
            escapeRoomHistoryService.getThemeHistories(
                themeId,
                currentPage,
                pageSize
            ),
    ...
    const getSuccessStatusBadge = (status: string) => {
        switch (status) {
            ...
            case "PARTIAL":
                return <Badge variant="secondary">부분 성공</Badge>;
            default:
                return null;
        }
    };
    ...
    const fetchHistoryComments = async (historyId: string) => {
        try {
            const response = await escapeRoomCommentService.getCommentsByTheme(
                themeId,
                0,
                100
            );
            const historyComment = response.content.find(
                (comment) =>
                    comment.escapeRoomHistoryId === historyId &&
                    comment.isGameHistoryComment
            );

            if (historyComment) {
                setHistoryComments((prev) =>
                    new Map(prev).set(historyId, [historyComment])
                );
            }
        } catch (error) {
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            <div className="text-center p-3 border rounded-lg">
                                <div className="text-2xl font-bold">
                                    {statsData.averageParticipants?.toFixed(
                                        1
                                    ) || "-"}
                                    명
                                </div>
                        ...
                        <div className="mt-4 pt-4 border-t">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {statsData.averageFeltDifficulty !==
                                    undefined &&
                                    statsData.averageFeltDifficulty !==
                                        null && (
                                ...
                                {statsData.averageFunRating !== undefined &&
                                    statsData.averageFunRating !== null && (
                                        <div className="flex items-center justify-between">
                                            <span className="text-sm text-gray-600">
                                                평균 재미
                                            </span>
                                ...
                                {statsData.averageStoryRating !== undefined &&
                                    statsData.averageStoryRating !== null && (
                                        <div className="flex items-center justify-between">
                                            <span className="text-sm text-gray-600">
                                                평균 스토리
                                            </span>
                            ...
                            <div className="mt-2 text-center">
                                {statsData.averageHintUsed !== undefined &&
                                    statsData.averageHintUsed !== null && (
                                        <>
                                            <span className="text-sm text-gray-600">
                                                평균 힌트:{" "}
                                            </span>
                                            <span className="font-medium">
                                                {statsData.averageHintUsed.toFixed(
                                                    1
                                                )}
                                                개
                                            </span>
                ...
                {histories.length === 0 ? (
                    <div className="text-center py-8">
                        ...
                        {onAddGameHistory && (
                            ...
                            >
                                <Trophy className="w-4 h-4 mr-2" />첫 플레이
                                기록 추가
                            </Button>
                        )}
                    </div>
                    ...
                    <div className="space-y-3">
                        {histories.map((history) => {
                            const isSpoilerVisible = showSpoilers.has(
                                history.id
                            );
                            const shouldShowSpoiler =
                                history.isSpoiler && !history.isOwn;

                            return (
                                <Card
                                    key={history.id}
                                    className={
                                        history.isOwn ? "border-primary" : ""
                                    }
                                >
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex items-start justify-between">
                                                <div className="flex items-center gap-3 flex-wrap">
                                                    ...
                                                    {getSuccessStatusBadge(
                                                        history.successStatus
                                                    )}
                                                    {history.isSpoiler && (
