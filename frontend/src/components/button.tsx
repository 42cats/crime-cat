...
export function Button({
    ...
    const buttonStyles = {
        container:
            "bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 transition-all duration-200",
        expanded: "shadow-elevated dark:shadow-black/20",
        collapsed:
            "shadow-subtle dark:shadow-none hover:shadow-depth dark:hover:shadow-md",
        ...
        summary:
            "text-sm text-zinc-500 dark:text-zinc-400 truncate-2-lines mt-1",
        buttonIcon:
            "h-8 w-8 text-zinc-400 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-700",
        chevron: "h-5 w-5 text-zinc-500 dark:text-zinc-400",
        content: "p-4 space-y-4 animate-slide-up",
        addButton:
            "w-full mt-4 border-dashed flex items-center gap-2 border-zinc-300 dark:border-zinc-600 hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-200",
        addIcon: "h-4 w-4",
        removeButton:
    ...
    const handleCompositionEnd = (
        ...
        handleButtonNameInput(
            e as unknown as React.ChangeEvent<HTMLInputElement>
        );
    };

    // 개선: 한 번만 전체 채널을 읽어와 id→name 맵을 만들기
    useEffect(() => {
        const loadAllChannels = async () => {
            try {
                const all = await fetchChannels(guildId);
                // ↓ 초기값을 as Record<string,string> 로 강제 캐스팅
                const mapNames = all.reduce(
    ...
    const removeContent = (contentId: string) => {
        const updatedContents = data.contents.filter(
    ...
    const isDropTarget =
        ...
        (dropIndicator.newIndex === index ||
            dropIndicator.newIndex === index + 1);
            ...
            >
                ...
                >
                    ...
                    {isEditing ? (
                        ...
                        <>
                            <div className="flex-1">
                                <h3 className={buttonStyles.title}>
                                    {data.name}
                                </h3>
                    ...
                    <UIButton
                        ...
                        aria-label={
                            isExpanded ? "Collapse button" : "Expand button"
                        }
                    >
                        {isExpanded ? (
