...
const AdvertisementStatsTable: React.FC<AdvertisementStatsTableProps> = ({ stats }) => {
      ...
      <CardContent>
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {stats.map((stat) => (
                <TableRow key={stat.requestId} className="hover:bg-muted/50">
                  ...
                  <TableCell className="text-center">
                    <span className={`font-medium ${
                      stat.clickThroughRate > 5 ? 'text-green-600' :
                      stat.clickThroughRate > 2 ? 'text-blue-600' :
                      'text-muted-foreground'
                    }`}>
                      {formatPercentage(stat.clickThroughRate)}
                    </span>
                  </TableCell>
                  <TableCell className="text-center">
                    <div className="flex items-center justify-center gap-1">
                  ...
                  <TableCell className="text-center">
                    <div className="flex items-center justify-center gap-1 text-sm">
                      ...
                      {stat.status === 'ACTIVE' && stat.remainingDays && (
                        <span className="text-xs text-blue-600">
                          (남은: {stat.remainingDays}일)
                        </span>
                      )}
                    </div>
                  </TableCell>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">
              {stats.reduce((sum, stat) => sum + stat.exposureCount, 0).toLocaleString()}
            </div>
            <div className="text-sm text-muted-foreground">총 노출수</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">
              {stats.reduce((sum, stat) => sum + stat.clickCount, 0).toLocaleString()}
            </div>
            <div className="text-sm text-muted-foreground">총 클릭수</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600">
              {formatCurrency(stats.reduce((sum, stat) => sum + stat.totalCost, 0))}P
            </div>
