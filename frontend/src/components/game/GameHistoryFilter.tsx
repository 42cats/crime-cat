...
import { Search, SortAsc, Filter, Calendar, CheckCircle2, XCircle } from 'lucide-react';
...
const GameHistoryFilter: React.FC<GameHistoryFilterProps> = ({
  ...
  const getFilterSummary = (): string => {
    ...
    if (startDate && endDate) {
      filters.push(`${format(startDate, 'yy.MM.dd')}~${format(endDate, 'yy.MM.dd')}`);
    } else if (startDate) {
  ...
  // 필터가 적용되었는지 확인
  const hasActiveFilters = winFilter !== null || startDate !== null || endDate !== null || hasTheme !== null;
  ...
  return (
    <div className="flex flex-col space-y-3 md:space-y-0 md:flex-row md:items-center w-full mb-4">
      ...
      <div className="flex items-center justify-end gap-2 md:ml-auto">
              ...
              <DropdownMenuPortal>
                <DropdownMenuSubContent>
                  <DropdownMenuCheckboxItem
                    ...
                    onCheckedChange={() => onWinFilterChange(winFilter === true ? null : true)}
                  >
                  ...
                  <DropdownMenuCheckboxItem
                    ...
                    onCheckedChange={() => onWinFilterChange(winFilter === false ? null : false)}
                  >
              ...
              <DropdownMenuPortal>
                <DropdownMenuSubContent className="p-2">
                  <div className="flex flex-col space-y-2">
                    <div className="text-xs font-medium">시작일</div>
                    <Popover>
                      <PopoverTrigger asChild>
                        ...
                        >
                          ...
                          {startDate ? format(startDate, 'PPP', { locale: ko }) : '선택하세요'}
                        </Button>
                      </PopoverTrigger>
                    ...
                    <div className="text-xs font-medium">종료일</div>
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button
                          variant="outline"
                        ...
                        >
                          <Calendar className="mr-2 h-3.5 w-3.5" />
                          {endDate ? format(endDate, 'PPP', { locale: ko }) : '선택하세요'}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0" align="start">
                        <CalendarComponent
              ...
              <DropdownMenuPortal>
                <DropdownMenuSubContent>
                  <DropdownMenuCheckboxItem
                    ...
                    onCheckedChange={() => onHasThemeChange(hasTheme === true ? null : true)}
                  >
                    <span>테마 있음</span>
                  </DropdownMenuCheckboxItem>
                  <DropdownMenuCheckboxItem
                    checked={hasTheme === false}
                    onCheckedChange={() => onHasThemeChange(hasTheme === false ? null : false)}
                  >
                    <span>테마 없음</span>
                  </DropdownMenuCheckboxItem>
                </DropdownMenuSubContent>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="h-9 flex items-center gap-1 text-xs">
          ...
          <DropdownMenuContent align="end" className="w-[160px]">
            <DropdownMenuRadioGroup value={sortType} onValueChange={onSortChange}>
              ...
              <DropdownMenuRadioItem value="CHARACTERNAME" className="text-xs">캐릭터명순</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="WIN_FIRST" className="text-xs">승리 먼저</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="LOSE_FIRST" className="text-xs">패배 먼저</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="THEME_NAME" className="text-xs">테마명순</DropdownMenuRadioItem>
            </DropdownMenuRadioGroup>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
