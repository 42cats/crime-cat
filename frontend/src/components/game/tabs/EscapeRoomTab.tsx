...
export const EscapeRoomTab: React.FC<EscapeRoomTabProps> = ({
    initialFilter,
}) => {
    const { user } = useAuth();
    ...
    const { data, isLoading, error, refetch } =
        useQuery<IntegratedGameHistoryResponse>({
            ...
            queryFn: () =>
                integratedHistoryService.getEscapeRoomHistories(
                    user!.id,
                    filter
                ),
            enabled: !!user?.id,
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    <Card>
                        <CardContent className="p-4">
                            ...
                            <p className="text-2xl font-bold">
                                {data.statistics.escapeRoom.playCount} /{" "}
                                {data.statistics.escapeRoom.total}
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-4">
                            ...
                            <p className="text-2xl font-bold">
                                {data.statistics.escapeRoom.unique}
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-4">
                            ...
                            <p className="text-2xl font-bold text-green-600">
                                {data.statistics.escapeRoom.winCount}
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                    ...
                    <Card>
                        <CardContent className="p-4">
                            ...
                            <p className="text-2xl font-bold">
                                {data.statistics.escapeRoom.winRate.toFixed(1)}%
                            </p>
                        </CardContent>
                    </Card>
                </div>
                        ...
                        <div className="space-y-4">
                            {data.escapeRoomHistories.map((history) => (
                                <EscapeRoomHistoryCard
                                    ...
                                    onClick={() =>
                                        navigate(
                                            `/themes/escape-room/${history.escapeRoomThemeId}`
                                        )
                                    }
                                />
                            ))}
