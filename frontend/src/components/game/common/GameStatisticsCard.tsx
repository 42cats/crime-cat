    ...
    <Card className={cn("overflow-hidden", className)}>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-700 mb-3">전체 통계</h3>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-gray-500">
              {crimeSceneStats.winCount}/{crimeSceneStats.total} 승리
            </span>
          </div>
          <div className="space-y-3">
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-gray-700 font-medium">{crimeSceneStats.winRate.toFixed(1)}%</span>
              </div>
              <Progress 
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-gray-500">
              {escapeRoomStats.winCount}/{escapeRoomStats.total} 성공
            </span>
          </div>
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-4">
              <StatItem
                ...
                label="플레이 횟수"
                value={escapeRoomStats.total}
                subValue={`(${escapeRoomStats.unique}개 테마)`}
              />
              <StatItem
                icon={<Trophy className="w-4 h-4 text-green-600" />}
                label="성공률"
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-gray-700 font-medium">{escapeRoomStats.winRate.toFixed(1)}%</span>
              </div>
              <Progress 
            ...
            {escapeRoomStats.total > escapeRoomStats.unique && (
              <div className="text-xs text-gray-500 bg-gray-50 rounded p-2">
                평균 {(escapeRoomStats.total / escapeRoomStats.unique).toFixed(1)}회 재플레이
              </div>
            )}
          </div>
        </div>
        ...
        <div className="border-t pt-4">
          <h3 className="text-sm font-medium text-gray-700 mb-3">게임별 비교</h3>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600">크라임씬</span>
              <div className="flex-1 mx-3">
                <Progress 
                  value={(crimeSceneStats.total / statistics.totalPlayCount) * 100} 
                  className="h-2"
                  indicatorClassName="bg-indigo-500"
                />
              </div>
              <span className="text-sm font-medium">{crimeSceneStats.total}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600">방탈출</span>
              <div className="flex-1 mx-3">
                <Progress 
                  value={(escapeRoomStats.total / statistics.totalPlayCount) * 100} 
                  className="h-2"
                  indicatorClassName="bg-blue-500"
                />
              </div>
              <span className="text-sm font-medium">{escapeRoomStats.total}</span>
            </div>
          </div>
        </div>
      </CardContent>
