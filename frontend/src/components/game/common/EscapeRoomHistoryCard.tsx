...
import { EscapeRoomHistoryDetailResponse, SuccessStatus } from '@/types/integratedGameHistory';
    ...
    >
      <div className="flex">
        ...
        <div className="relative w-48 h-36 flex-shrink-0">
          <img
            src={history.themeThumbnail || '/images/escape-room-default.jpg'}
        ...
        <div className="flex-1 p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
            ...
            <div className="flex items-center gap-1.5 text-sm">
              ...
              <span>{format(new Date(history.playDate), 'yyyy.MM.dd', { locale: ko })}</span>
            </div>
            ...
            {history.clearTime && (
              <div className="flex items-center gap-1.5 text-sm">
                ...
                <span>{history.formattedClearTime || `${history.clearTime}분`}</span>
              </div>
            )}
          ...
          <div className="flex items-center gap-4 mb-3">
            ...
            {history.funRating && (
              <div className="flex items-center gap-1">
                <span className="text-sm text-gray-600">재미</span>
                <div className="flex">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
            ...
            {history.storyRating && (
              <div className="flex items-center gap-1">
                <span className="text-sm text-gray-600">스토리</span>
                <div className="flex">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
          ...
          {/* 클리어 타임 진행바 */}
          {history.clearTime && history.successStatus === SuccessStatus.SUCCESS && (
            <div className="mb-3">
              <div className="flex justify-between items-center mb-1">
                <span className="text-xs text-gray-600">클리어 타임</span>
                <span className="text-xs text-gray-600">{history.clearTime}분 / 60분</span>
              </div>
              <Progress value={clearTimeProgress} className="h-2" />
            </div>
          )}
          ...
          {history.memo && (
            <p className={cn(
              ...
              history.isSpoiler && "blur-sm hover:blur-none transition-all"
            )}>
              {history.memo}
            </p>
          )}
          ...
          <div className="flex justify-between items-center mt-3 pt-3 border-t">
            <div className="flex items-center gap-2">
              <Avatar className="w-6 h-6">
                ...
                <AvatarFallback>{history.userNickname?.[0]}</AvatarFallback>
              </Avatar>
              <span className="text-sm text-gray-600">{history.userNickname}</span>
            </div>
