      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            <div>
              ...
              {comment.createdAt !== comment.updatedAt && (
                <span className="text-xs text-muted-foreground ml-2">(수정됨)</span>
              )}
            </div>
          ...
          {isEditing ? (
            ...
            <>
              <div className={`mt-1 ${!canViewSpoiler ? 'blur-sm hover:blur-none cursor-pointer transition-all' : ''}`}>
                {!canViewSpoiler && (
                  <div className="flex items-center mb-1 text-amber-600">
                    <AlertTriangle className="h-4 w-4 mr-1" />
                    <span className="text-xs">스포일러 내용입니다. 게임을 플레이한 후 확인하세요.</span>
                  </div>
                )}
              ...
              <div className="flex items-center gap-4 mt-3">
                <Button 
                  variant="ghost" 
                  size="sm" 
                  className={`text-xs flex items-center gap-1 px-2 py-1 h-auto ${isLiked ? 'text-blue-600' : ''}`}
                  onClick={handleLike}
                >
