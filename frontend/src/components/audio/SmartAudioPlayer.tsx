    ...
    useEffect(() => {
        ...
        const fetchAudio = async () => {
            ...
            try {
                ...
                console.error(
                    "❌ SmartAudioPlayer - Failed to fetch audio:",
                    err
                );
                if (!isCancelled) {
                    // 더 친화적인 에러 메시지
                    let errorMsg =
                        "오디오를 가져오는 데 실패했습니다. 로그인을 확인해 주시고, 잠시 후 다시 시도해주세요.";

                    if (err instanceof Error) {
                        if (err.message.includes("메모리")) {
                            errorMsg =
                                "시스템 메모리 부족으로 잠시 후 다시 시도해주세요";
                        } else if (err.message.includes("🔒 로그인이 필요한")) {
                            errorMsg = "🔒 로그인이 필요한 오디오입니다";
                        }
                    }
                    setError(errorMsg);
                    setLoading(false);
        ...
        try {
            ...
            if (err instanceof Error) {
                if (err.message.includes("NotAllowedError")) {
                    errorMessage =
                        "브라우저에서 오디오 재생을 차단했습니다. 페이지를 클릭한 후 다시 시도해주세요.";
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between mb-3">
                ...
                {error && (
                    <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-md px-3 py-2 ml-2">
