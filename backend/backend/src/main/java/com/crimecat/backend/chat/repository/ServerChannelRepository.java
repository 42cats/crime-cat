...
public interface ServerChannelRepository extends JpaRepository<ServerChannel, UUID> {
    ...
    // 서버의 활성 채널 조회
    List<ServerChannel> findByServerIdAndIsActiveTrue(UUID serverId);
    
    Page<ServerChannel> findByServerIdAndIsActiveTrue(UUID serverId, Pageable pageable);

    // 서버 내에서 채널명으로 조회
    Optional<ServerChannel> findByServerIdAndNameAndIsActiveTrue(UUID serverId, String name);
    ...
    List<ServerChannel> findByServerIdAndChannelType(@Param("serverId") UUID serverId, @Param("channelType") ServerChannel.ChannelType channelType);
    ...
    List<ServerChannel> findTextChannelsByServerId(@Param("serverId") UUID serverId);
    ...
    List<ServerChannel> findVoiceChannelsByServerId(@Param("serverId") UUID serverId);
    ...
    List<ServerChannel> findByServerIdAndMemberCountGreaterThanEqual(@Param("serverId") UUID serverId, @Param("minMembers") int minMembers);

    // 채널 통계 조회 (멤버 포함)
    @Query("SELECT sc FROM ServerChannel sc LEFT JOIN FETCH sc.members m WHERE sc.id = :channelId AND sc.isActive = true")
    Optional<ServerChannel> findByIdWithMembers(@Param("channelId") UUID channelId);
    ...
    List<ServerChannel> findChannelsByServerIdAndUserId(@Param("serverId") UUID serverId, @Param("userId") UUID userId);

    // 인기 채널 조회 (멤버 수 기준)
    @Query("SELECT sc FROM ServerChannel sc WHERE sc.server.id = :serverId AND sc.isActive = true ORDER BY SIZE(sc.members) DESC")
    Page<ServerChannel> findPopularChannelsByServerId(@Param("serverId") UUID serverId, Pageable pageable);
    ...
    List<ServerChannel> findRecentlyActiveChannelsByServerId(@Param("serverId") UUID serverId, @Param("since") LocalDateTime since);
    ...
    List<ServerChannel> findFirstChannelByServerId(@Param("serverId") UUID serverId, Pageable pageable);
    
    // 서버의 활성 채널을 생성일시 순으로 조회
    List<ServerChannel> findByServerIdAndIsActiveTrueOrderByCreatedAt(UUID serverId);
    
    // 특정 서버의 특정 채널 조회
    Optional<ServerChannel> findByIdAndServerIdAndIsActiveTrue(UUID channelId, UUID serverId);
}