...
public interface ServerMemberRepository extends JpaRepository<ServerMember, UUID> {
    ...
    // 서버의 모든 활성 멤버 조회
    List<ServerMember> findByServerIdAndIsActiveTrue(UUID serverId);
    
    Page<ServerMember> findByServerIdAndIsActiveTrue(UUID serverId, Pageable pageable);

    // 특정 사용자가 특정 서버의 멤버인지 확인
    Optional<ServerMember> findByServerIdAndUserIdAndIsActiveTrue(UUID serverId, UUID userId);
    ...
    List<ServerMember> findAdminsByServerId(@Param("serverId") UUID serverId);
    ...
    Optional<ServerMember> findAdminByServerIdAndUserId(@Param("serverId") UUID serverId, @Param("userId") UUID userId);
    ...
    List<ServerMember> findRecentlyActiveMembersByServerId(@Param("serverId") UUID serverId, @Param("since") LocalDateTime since);
    ...
    List<ServerMember> findByServerIdAndRole(@Param("serverId") UUID serverId, @Param("role") ServerMember.ServerRole role);
    ...
    boolean isUserAdminOfServer(@Param("serverId") UUID serverId, @Param("userId") UUID userId);
    ...
    boolean isUserMemberOfServer(@Param("serverId") UUID serverId, @Param("userId") UUID userId);
    
    // 서버 멤버 존재 여부 확인
    boolean existsByServerIdAndUserIdAndIsActiveTrue(UUID serverId, UUID userId);
    ...
    List<ServerMember> findOnlineMembersByServerId(@Param("serverId") UUID serverId, @Param("onlineThreshold") LocalDateTime onlineThreshold);
}