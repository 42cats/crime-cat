...
public interface ServerRoleRepository extends JpaRepository<ServerRole, Long> {
    ...
    // 서버의 모든 활성 역할 조회
    List<ServerRole> findByServerIdAndIsActiveTrue(Long serverId);
    
    Page<ServerRole> findByServerIdAndIsActiveTrue(Long serverId, Pageable pageable);

    // 서버 내에서 역할명으로 조회
    Optional<ServerRole> findByServerIdAndNameAndIsActiveTrue(Long serverId, String name);
    ...
    // 색상별 역할 조회
    List<ServerRole> findByServerIdAndColorAndIsActiveTrue(Long serverId, String color);
    ...
    List<ServerRole> findByIdsAndServerId(@Param("roleIds") List<Long> roleIds, @Param("serverId") Long serverId);

    // 특정 권한들을 모두 가진 역할 조회
    @Query("SELECT sr FROM ServerRole sr WHERE sr.server.id = :serverId AND sr.isActive = true")
    List<ServerRole> findAllByServerId(@Param("serverId") Long serverId);
    ...
    List<ServerRole> findDefaultRolesByServerId(@Param("serverId") Long serverId);
    ...
    List<ServerRole> findCustomRolesByServerId(@Param("serverId") Long serverId);

    // 역할 생성 순서로 조회
    List<ServerRole> findByServerIdAndIsActiveTrueOrderByCreatedAt(Long serverId);

    // 역할명으로 정렬하여 조회
    List<ServerRole> findByServerIdAndIsActiveTrueOrderByName(Long serverId);

    // 특정 멤버가 가진 역할들 조회
    @Query("SELECT sr FROM ServerRole sr WHERE sr.id IN :roleIds AND sr.isActive = true")
    List<ServerRole> findByIdInAndIsActiveTrue(@Param("roleIds") List<Long> roleIds);

    // 삭제된 역할 조회 (복구용)
    List<ServerRole> findByServerIdAndIsActiveFalse(Long serverId);

    // 서버의 모든 역할 조회 (활성/비활성 모두)
    List<ServerRole> findByServerIdOrderByIsActiveDescCreatedAtAsc(Long serverId);
}