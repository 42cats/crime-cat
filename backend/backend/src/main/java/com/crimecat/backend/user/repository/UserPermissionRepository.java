...
public interface UserPermissionRepository extends JpaRepository<UserPermission, UUID> {
	...
	Optional<UserPermission> findUserPermissionByPermissionName(User user, Permission permission);

	@Query("SELECT up FROM UserPermission up JOIN FETCH up.user WHERE up.user = :user AND up.expiredAt > :now")
	List<UserPermission> getActiveUserPermissions(User user, LocalDateTime now);
