...
class WebUserCreationTest {
    ...
    @Order(1)
    void test1_정상_유저_생성() {
        WebUser user = discordOAuth2UserService.processOAuthUser(discordId, email, nickname, provider);
    ...
    void test2_중복_생성시_기존유저_반환() {
        WebUser first = discordOAuth2UserService.processOAuthUser(discordId, email, nickname,provider);
        WebUser second = discordOAuth2UserService.processOAuthUser(discordId, email, nickname, provider);
    ...
    void test3_닉네임_누락시_예외() {
        assertThatThrownBy(() -> discordOAuth2UserService.processOAuthUser(discordId, email, null, provider))
    ...
    void test5_이메일_중복시_예외_또는_갱신_정책_검증() {
        discordOAuth2UserService.processOAuthUser(discordId, email, nickname,provider);
        // 새로운 Discord ID지만 같은 이메일로 유저 생성 시 예외가 발생해야 하는 정책이라면:
        assertThatThrownBy(() ->
                discordOAuth2UserService.processOAuthUser("999999", email, "다른닉네임", provider))
                .isInstanceOf(RuntimeException.class)
    ...
    void test6_로그인방식이_GOOGLE으로_설정되는지() {
        WebUser user = discordOAuth2UserService.processOAuthUser(discordId, email, nickname,"google");
        assertThat(user.getLoginMethod()).isEqualTo(LoginMethod.GOOGLE);
    }
    @Test
    @Order(6)
    void test6_로그인방식이_LOCAL_으로_설정되는지() {
        WebUser user = discordOAuth2UserService.processOAuthUser(discordId, email, nickname,"local");
        assertThat(user.getLoginMethod()).isEqualTo(LoginMethod.GOOGLE);
    }
}
